...
try {
    ...
    // Try fetching one user
    if ($total > 0) {
        $stmt = $db->query("SELECT id, full_name, email, phone, user_type, email_verified, phone_verified, profile_image, is_banned, created_at FROM users LIMIT 1");
