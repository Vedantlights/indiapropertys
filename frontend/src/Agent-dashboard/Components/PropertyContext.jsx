  ...
  const fetchInquiries = useCallback(async (showLoading = false) => {
    ...
    try {
      ...
      if (response.success && response.data && response.data.inquiries) {
        const backendInquiries = response.data.inquiries.map(transformInquiry);
      ...
      setInquiries([]);
      setInquiriesError(error.message || 'Failed to fetch inquiries. Please check your connection.');
    } finally {
